...
export type Curriculum = { id: number; title: string; slug: string; status: string; sequential: boolean; issues_certificate: boolean; description_markdown: string | null; sections: Section[] };
...
export function CourseEditor({ initial }: { initial?: Curriculum }) {
  ...
  async function action(work: () => Promise<void>) {
    ...
    try {
      ...
      setError(caught instanceof ApiError ? [caught.message, ...Object.values(caught.fields).flat()].join(' ') : (bn ? 'সংরক্ষণ হয়নি। আবার চেষ্টা করুন।' : 'Could not save. Please try again.'));
  ...
  function saveCourse(event: React.FormEvent<HTMLFormElement>) {
    ...
    void action(async () => {
      const response = await api<{ data: { id: number } }>(course ? base : '/admin/courses', { method: course ? 'PATCH' : 'POST', body: { title: data.get('title'), slug: data.get('slug'), description_markdown: data.get('description_markdown'), sequential: data.get('sequential') === 'on', issues_certificate: data.get('issues_certificate') === 'on' } });
  ...
  return <div className="space-y-8">
    ...
    <form onSubmit={saveCourse} className="space-y-4 rounded-xl border border-line bg-white p-5">
      <h2 className="text-lg font-bold text-navy">{bn ? 'কোর্সের তথ্য' : 'Course details'}</h2>
    ...
    {course ? <>
      ...
      <section className="space-y-4"><h2 className="text-xl font-bold text-navy">{bn ? 'কোর্সের অধ্যায় ও পাঠ' : 'Sections & lessons'}</h2>
        {course.sections.map((section) => <div key={section.id} className="rounded-xl border border-line bg-white p-5">
          <form onSubmit={(event) => saveSection(event, section)} className="flex flex-wrap items-end gap-3"><label className="flex-1">{bn ? 'অধ্যায়ের নাম' : 'Section title'}<input required name="title" defaultValue={section.title} className={input} /></label><label className="w-28">{bn ? 'কত দিন পরে' : 'Drip days'}<input name="drip_days" type="number" min="0" max="3650" defaultValue={section.drip_days ?? ''} className={input} /></label><Button type="submit" disabled={busy}>{bn ? 'সংরক্ষণ' : 'Save'}</Button></form>
