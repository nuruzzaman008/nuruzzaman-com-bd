...
sync_tree() {
  ...
  for sync_arg in "$@"; do
    case "$sync_arg" in
      --exclude=*)
        sync_pattern="${sync_arg#--exclude=}"
        # rsync reads a trailing slash as "directories only". GNU tar matches
        # nothing at all against such a pattern - it does not warn, it simply
        # copies the tree the exclude was there to protect, which is how
        # storage/, bootstrap/cache/ and tests/ have been shipped over the
        # live application all along. Stripping the slash makes tar exclude
        # the directory and everything under it, which is what the slash asks
        # for. infra/cpanel/tests/sync-tree.test.sh holds the two to it.
        sync_excludes+=("--exclude=${sync_pattern%/}")
        ;;
    esac
  done
  mkdir -p "$sync_dest"
